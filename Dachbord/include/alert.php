...
<section>
	...
	<?php
	$sql = "SELECT Status,RegCode FROM user WHERE UserId = ?";
	$stmt = $conn->prepare($sql);
	$stmt->bind_param("i", $UserId);
	$stmt->execute();
	$reusalt = $stmt->get_result();
	if ($reusalt->num_rows > 0 && $row = $reusalt->fetch_assoc()) {
		$UserRegCode = $row['RegCode'];
	}
	$stmt->close();
	?>
	<div class="modal-box">
		<i onclick="close_alert()" class="fa-solid fa-xmark close-x"></i>
		<input type="hidden" id="hiddenRegCode" value="<?php echo  $UserRegCode; ?>">
		<div class="row w-100 m-0">
			<h2 id="instiName" style="text-align: center">Register Institute</h2>
			<div class="col-sm-4 col-12 w-100">
				<div class="card">
					<div class="card-body p-2">
						<label class="form-label">Institute Id Numner</label>
						<input id="instiid" type="text" class="form-control" placeholder="">
					...
					<div class="row w-100 m-0">
						...
						<div class="col-sm-4 col-12 w-100 m-0">
							...
							<div class="card-body p-2 pt-0 PaymentPhy">
								<label class="form-check-label mt-4 text-dark" for="uploadslip">Upload Institute Payment slip</label>
								<input type="file" value="" id="uploadslip" class="form-control mt-1" placeholder="Upload Institute Payment slip" accept="image/*">
								<div id="invalid_feed" class="invalid-feedback">Plece Upload Institute Payment slip</div>
							</div>
						</div>
					</div>
					<!-- </center> -->
